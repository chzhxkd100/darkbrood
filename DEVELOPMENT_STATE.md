...
> 3. **자가 검증 배포 및 능동적 검증 (Self-Verifying Work & Proactive Testing)**:
>    - 빌드나 배포 이슈를 수정할 때는 사용자가 수동으로 진행 상황을 확인하도록 요구하지 마십시오. **AI 스스로 API 폴링용 일회성 스크립트(Node.js 등)를 작성하여 실행하거나, 브라우저 대리인 도구(Browser Subagent)를 띄워 GitHub Actions 페이지의 실시간 로그를 끝까지 모니터링하여 성공/실패 결과를 스스로 분석하고 조치해야 합니다.**
>    - API 호출 제한(Rate Limit) 등 장벽이 발생하면 즉시 브라우저 컨트롤 도구를 이용해 웹페이지의 렌더링된 텍스트 및 로그를 스크래핑 및 시각적으로 확인해 대응 방식을 끊임없이 업데이트해야 합니다.
> 4. **점진적 깃 커밋/푸시 및 배포 검증 (Incremental Commits & Continuous Integration)**:
>    - 논리적 단위의 기능 개발이나 오류 수정이 완료될 때마다 즉시 **Git 커밋 후 원격(origin main)으로 푸시하여 CI/CD 배포 파이프라인을 작동시키고 무중단 서비스에 정상 배포되는 것까지 직접 단계별로 확인**해야 합니다. 한 번에 너무 방대한 변경 사항을 모아서 푸시하는 것을 지양하십시오.
...
### 1) 신전 (Notice & Intro - `/`)
- **설명**: 메인 대시보드로, 데이터베이스의 `posts` 컬렉션 중 `type: 'notice'` 필터링 공지사항을 노출하고 관리자 인트로 메시지를 출력합니다. 최근 안내 사항은 최신순 5개 노출 제한이 적용됩니다.
...
실시간 채팅은 주기적인 폴링(Cursor-based polling) 구조 및 역무한 스크롤(Reverse Infinite Scroll) 페이징을 결합하여 지원하며, 사이드바 위젯과 독립형/OBS 브라우저 소스 전용 화면으로 이원화되어 있습니다.
...
- **독립형 및 OBS 오버레이 (`GET /chat` & `views/chat.ejs`)**:
  ...
  - **OBS 오버레이 (`/chat?overlay=true`)**: OBS Studio 등 방송 프로그램의 "브라우저 소스"에 최적화된 오버레이 뷰.
    - 입력 폼과 제목 표시줄, 타임스탬프가 숨겨지고, 배경이 완전히 투명(`background: transparent`)해집니다.
    - 채팅 형식은 불필요한 레이아웃 없이 **`닉네임: 채팅내용`** 형태로만 아주 단순하게 출력됩니다.
    - 대기실의 "심연과 동기화 중..." 또는 "침묵만이 흐르는..." 등의 플레이스홀더 텍스트를 제거하여 채팅이 없는 공백 상태에서는 화면이 100% 투명한 공백을 유지하도록 처리되었습니다.
...
5. **Firebase Hosting 배포 오류 방지 및 버전/환경 고정**
   ...
   - **이슈 2 (OAuth 토큰 발급 시 Premature close 에러)**: Node.js 2026년 6월 보안 업데이트(Node 20.x, 22.x 최신 마이너 버전)로 인한 `http.Agent` 동작 변화로, Firebase CLI 내부의 `node-fetch` 모듈이 구글 OAuth 토큰 발급 시 연결을 강제 종료(`Premature close`)하며 인증 실패를 유발합니다.
      - **중요 해결책**: GitHub Actions에서 자바스크립트 액션(예: `action-hosting-deploy`)을 사용하면 러너 호스트의 기본 Node.js 버전을 따르기 때문에 `actions/setup-node` 설정이 무시됩니다. 따라서 써드파티 액션을 걷어내고 **일반 쉘 스크립트 실행(`run`) 단계**로 대체하여 `npx firebase-tools@15.20.0 deploy`를 직접 구동시켰습니다.
      - **Node 22.2.0 고정 및 CLI v15.20.0 선택 사유**: Node `20.15.0` + CLI `13.15.0` 환경에서 `universal-analytics` 등 패키지가 Node >= 22 요건으로 인해 `ERR_REQUIRE_ESM` 에러를 일으켰습니다. 이를 피하기 위해 Node >= 22 요건을 충족하면서 June 2026 보안 패치 이전 버전인 **`Node 22.2.0`**을 사용하고, CLI 버전은 중복 릴리즈 버그가 발생하기 직전의 가장 안정적인 최신 릴리즈인 **`firebase-tools@15.20.0`**으로 상향 조정했습니다.
